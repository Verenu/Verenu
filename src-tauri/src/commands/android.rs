...
pub fn android_keystore_save(provider: String, key: String) -> Result<(), String> {
    ...
    crate::android::provide_credential(&provider, &normalized);
...
/// settings). Kotlin reports the outcome via
/// `verenu:android-permission-snapshot`; this command only dispatches.
#[tauri::command]
...
mod tests {
    ...
    fn keyboard_visibility_command_drives_overlay() {
        ...
        let flight =
            android_on_keyboard_visibility(false, false, Some(OverlayState::Recording));
