...
pub fn set(provider: &str, key: &str) -> Result<(), String> {
    let (canonical_provider, normalized_key) = crate::android::credential_input(provider, key)?;
    if user_for(&canonical_provider).is_none() {
        return Err(format!("Unknown provider: {provider}"));
    }
    crate::android::provide_credential(&canonical_provider, &normalized_key);
    Ok(())
}
...
pub fn migrate_from_store(_app: &AppHandle, settings: &crate::data::store::SettingsHandle) {
    ...
    for (provider, store_key) in [
        ...
        if !already_migrated && !plaintext.is_empty() {
            ...
            // On Android `set` is memory-only; stage the rotation so Kotlin
            // persists it to the Keystore — otherwise the migrated key would
            // vanish on the next reboot while the plaintext is already gone.
            #[cfg(target_os = "android")]
            crate::android::bridge::stage_keystore_rotation(provider, &plaintext);
