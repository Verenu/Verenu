...
pub const ANDROID_TARGET_SDK: u32 = 34;
...
pub fn provide_credential(provider: &str, key: &str) {
    ...
    if key.is_empty() {
        cache.remove(&cache_key(provider));
    } else {
        cache.insert(cache_key(provider), key);
    }
}
...
mod tests {
    ...
    fn keyboard_hide_clears_idle_but_preserves_flight() {
        ...
        assert_eq!(OverlayState::Hidden.on_keyboard_hidden(), OverlayState::Hidden);
        assert_eq!(
    ...
    fn permission_rationale_covers_every_gate() {
        for permission in [
            ...
            assert!(rationale.len() > 40, "{permission:?} needs a real rationale");
        }
