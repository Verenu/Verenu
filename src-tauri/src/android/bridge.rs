...
//! | POST | `/v1/recording/stop` | → `{ ok, text }` (transcribed text, may be empty) |
//! | POST | `/v1/recording/cancel` | → `{ ok }` |
//! | POST | `/v1/recording/retry` | → `{ ok }` |
//! | POST | `/v1/insertion/ack` | `{ seq, success, strategy, error?, package? }` → `{ ok }` |
...
pub(crate) fn peek_outbox() -> Option<OutboxSnapshot> {
    ...
    guard.current.as_ref().map(|entry| OutboxSnapshot {
        ...
        age_ms: entry.published_at.elapsed().as_millis().min(u64::MAX as u128) as u64,
    })
}
...
pub(crate) fn parse_head(head: &str, token: &str) -> Result<(String, String, usize, bool), String> {
    ...
    let authorized = header_value(head, "authorization")
...
fn state_payload(state: &BridgeState) -> Value {
    ...
    let pending = peek_outbox().map(|snap| {
        json!({ "seq": snap.seq, "text": snap.text, "ageMs": snap.age_ms })
    });
    json!({
        "lifecycle": lifecycle,
...
pub(crate) fn note_pill_stage(stage: &str) {
    *pill_stage_cell()
        .lock()
        .unwrap_or_else(|e| e.into_inner()) = stage.to_string();
}
...
async fn handle_request(state: &BridgeState, req: HttpRequest) -> Vec<u8> {
    ...
    let bool_field = |name: &str| body_json.get(name).and_then(|v| v.as_bool()).unwrap_or(false);
    ...
    match (req.method.as_str(), req.path.as_str()) {
        ...
        ("POST", "/v1/recording/start") => {
            ...
            *state
                .package_hint
                .lock()
                .unwrap_or_else(|e| e.into_inner()) = package;
            let Some(app) = state.app.clone() else {
                return err(503, "Service Unavailable", "recording unavailable in this context");
            };
        ...
        ("POST", "/v1/recording/stop") => {
            ...
            let action = app.state::<crate::pipeline::SharedState>();
            match crate::commands::stop_and_transcribe_input(app.clone(), action).await {
                Ok(text) => ok(json!({ "chars": text.chars().count() })),
                Err(e) => err(500, "Internal Server Error", &e),
            }
        }
        ...
        ("POST", "/v1/insertion/ack") => {
                ...
                AckOutcome::Stale { expected } => {
                    return err(
                        ...
                        &format!(
                            ...
                            expected.map(|s| s.to_string()).unwrap_or_else(|| "none".into())
                        ),
                    );
                }
            ...
            log::info!(
                "android bridge: insertion ack seq={seq} success={success} strategy={} package={} error={}",
        ...
        ("POST", "/v1/credential") => {
            ...
            if provider.trim().is_empty() {
                return err(400, "Bad Request", "unknown provider");
            }
...
async fn serve_connection(
    ...
    let mut byte = [0u8; 1];
    // Read until end-of-head, bounded so a junk connection can't OOM us.
    let head_end = loop {
...
fn write_connection_file(port: u16, token: &str) -> anyhow::Result<()> {
    ...
    let payload =
        serde_json::to_vec(&json!({ "port": port, "token": token, "updatedUnixMs": now_unix_ms() }))?;
...
pub fn start_bridge(app: AppHandle) -> anyhow::Result<std::net::SocketAddr> {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        log::info!("android bridge: listening on {addr}");
        loop {
...
mod tests {
    ...
    async fn bridge_state_reports_no_pending_insertion_initially() {
        ...
        let (addr, token) = spawn_test_server().await;
        let (status, body) =
            roundtrip(addr, &authed(&token, "GET", "/v1/state", "")).await;
    ...
    async fn bridge_insertion_handoff_round_trips() {
        ...
        let (status, _) = roundtrip(
            addr,
            &authed(&token, "POST", "/v1/insertion/ack", r#"{"seq":9999,"success":true}"#),
        )
        .await;
        ...
        let (status, ack) = roundtrip(addr, &authed(&token, "POST", "/v1/insertion/ack", &body)).await;
        assert_eq!(status, 200);
    ...
    async fn bridge_recording_routes_need_an_app() {
        ...
        let (addr, token) = spawn_test_server().await;
        for path in ["/v1/recording/start", "/v1/recording/stop", "/v1/recording/cancel"] {
            let (status, _) =
                roundtrip(addr, &authed(&token, "POST", path, "{}")).await;
    ...
    async fn bridge_credential_push_lands_in_memory_cache() {
        ...
        let (status, _) = roundtrip(
            addr,
            &authed(&token, "POST", "/v1/credential", r#"{"provider":"groq","key":"gsk-x"}"#),
        )
        .await;
        assert_eq!(status, 200);
    ...
    async fn bridge_keystore_rotation_delivers_once() {
        ...
        let (_, empty) =
            roundtrip(addr, &authed(&token, "GET", "/v1/keystore/pending", "")).await;
    ...
    async fn bridge_rejects_malformed_and_unknown() {
        ...
        let (status, _) = roundtrip(
            addr,
            &authed(&token, "POST", "/v1/focus", "not-json{{{"),
        )
        .await;
        ...
        let (status, _) =
            roundtrip(addr, &authed(&token, "POST", "/v1/definitely-not-a-route", "{}")).await;
