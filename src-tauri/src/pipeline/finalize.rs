...
pub(super) async fn finalize_pipeline_completion(
    ...
    let (final_text_substituted, delivered_text, private_text, clipboard_warning) =
        if let Some(plan) = ctx.clipboard_plan {
            let marker_survived = plan.markers.iter().all(|marker| {
                final_text_substituted.matches(marker).count() == 1
            });
            ...
            let delivered = super::clipboard_phrase::restore(&marker_text, plan).unwrap_or_else(|| {
                ...
                plan.markers.iter().fold(plan.pre_cleanup.clone(), |text, marker| {
                    text.replace(marker, &plan.clipboard_text)
                })
            });
            let private = super::clipboard_phrase::private_text(&marker_text, plan);
            (marker_text, delivered, private, if !marker_survived { Some("Clipboard inserted without cleanup") } else { ctx.clipboard_warning })
        } else {
    ...
    let injected = if ctx.event_only {
        ...
        match injection::inject_text(
            ...
            ctx.cfg.contextual_caps_enabled && !apply_caps_lock_upper && !clipboard_is_leading,
