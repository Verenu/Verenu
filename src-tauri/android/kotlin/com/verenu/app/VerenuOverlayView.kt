 ...
 * 999px radius, 11px semibold stage labels, 150–220ms motion feel) in both
 * color schemes; controls adapt to touch (48dp minimum hit areas) without
 * changing the pill's visual footprint.
 *
...
class VerenuOverlayView @JvmOverloads constructor(
    ...
    init {
        ...
        wave = WaveView(context).apply {
            layoutParams = LinearLayout.LayoutParams((96 * density).toInt(), (24 * density).toInt())
        }
        ...
        // The whole pill is a 48dp+ tap target (tap = start/stop).
        isClickable = true
        isFocusable = false
    ...
    private fun pillButton(icon: Int, description: String): ImageButton =
        ImageButton(context).apply {
            ...
            background = null
            // 48dp hit area; the artwork stays small.
    ...
    private fun render() {
        row.removeAllViews()
        val normalBg = state != State.ERROR && state != State.CANCELLED
        (background as? android.graphics.drawable.GradientDrawable)?.setColor(
            if (state == State.ERROR) errorBg else pillBg,
        )
