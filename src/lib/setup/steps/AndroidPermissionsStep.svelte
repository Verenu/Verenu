...
<div class="android-perms">
	{#if !isAndroid}
		<p class="preview-note" role="note">
			Android permission preview — on a device each row opens the real system prompt.
				...
				{#if grant !== 'granted'}
					...
					<div class="perm-actions">
						<button
							...
							onclick={() => request(id)}
						>
							{requesting === id ? 'Opening…' : grant === 'not_asked' ? 'Allow' : 'Open Settings'}
						</button>
						<span class="perm-target-hint">{PERMISSION_SETTINGS_TARGET[id]}</span>
					</div>
				{/if}
			</li>
		{/each}
